...
(function (document, window) {
    ...
    angular.module('googlechart', ['ngResource'])
        ...
        .directive('googleChart', ['$timeout', '$window', '$rootScope', 'googleChartApiProxy', '$http', function ($timeout, $window, $rootScope, apiProxy, $http) {
            return {
                restrict: 'A',
                scope: {
                    chart: '=chart',
                ...
                link: function ($scope, $elm, $attr) {
                    ...
                    $scope.getChartWrapper = function() {
                      return $scope.chartWrapper;
                    }
                    
                    $scope.$on('sezione', function(event, e) {
                      var sezioneNome = e.sezione;
                      var dataTable = $scope.chartWrapper.getDataTable();
                      for(var i = 0; i < dataTable.getNumberOfRows(); i++) {
                        for (var j = 0; j < dataTable.getNumberOfColumns(); j++) {
                          if(dataTable.getValue(i, j).toString().match(sezioneNome)) {
                              $scope.chartWrapper.getChart().setSelection([{row:i, column:null}]);
                              break;
                          }
                        }
                      }
                        ...
                        if (!draw.triggered && ($scope.chart != undefined)) {
                            ...
                            $timeout(function () {
                                ...
                                if ($scope.chart.data instanceof google.visualization.DataTable) {
                                  ...
                                  dataTable = new google.visualization.arrayToDataTable($scope.chart.data, false);
                                }
